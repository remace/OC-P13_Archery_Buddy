...
class Arrow(models.Model):
    ...
    feathering = models.ForeignKey(
        "alternativeequipment.Feathering", on_delete=models.CASCADE
    )
    tip = models.ForeignKey("alternativeequipment.Tip", on_delete=models.CASCADE)
    tube = models.ForeignKey("alternativeequipment.Tube", on_delete=models.CASCADE)
    not_broken = models.BooleanField("en état d'utilisation", default=True)

    def __str__(self):
        return (
            f"{self.id}: {self.tube.brand} - {self.feathering.cock_color}"
            f"/{self.feathering.color} - {self.tube.tube_length}"
        )
